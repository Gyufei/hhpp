...
export const PrevButton = ({ className, ...buttonProps }: ButtonProps) => {
  ...
  return (
    <button
      {...buttonProps}
      className={cn(
        "flex h-8 w-8 items-center justify-center rounded bg-[#222428] text-xs leading-[18px] text-gray data-[active=true]:bg-main data-[active=true]:text-main",
        className,
      )}
      onClick={() => previous()}
...
const TruncableElement = ({ prev }: ITruncableElementProps) => {
  ...
  return (isPreviousTruncable && prev === true) ||
    ...
    <li>
      <a
        className={cn(
          "flex h-8 w-8 items-center justify-center rounded bg-[#222428] text-xs leading-[18px] text-gray data-[active=true]:bg-main data-[active=true]:text-bg-black",
        )}
      >
        ...
      </a>
  ...
  const renderPageButton = (page: number) => (
    <li key={page}>
      <a
        ...
        className={cn(
          "flex h-8 w-8 cursor-pointer items-center justify-center rounded bg-[#222428] text-xs leading-[18px] text-gray data-[active=true]:bg-main data-[active=true]:text-bg-black",
          className,
        )}
      >
        {page}
