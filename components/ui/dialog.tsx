...
>(({ className, showClose = true, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "w-full border-b border-border-black px-5 py-4 tracking-tight",
      className,
    )}
    {...props}
  >
    <div className="flex justify-start text-[18px] leading-[28px] text-title-white">
      ...
      <>
        {showClose && (
          <DialogPrimitive.Close className="absolute right-6 top-5 flex h-6 w-6 items-center justify-center rounded-full text-txt-white outline-0 transition-opacity disabled:pointer-events-none">
